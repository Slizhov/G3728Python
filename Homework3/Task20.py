...
word = input('Введите слово по-русски или по-английски: ')
if word.isalpha():
    russian_counts=0
    english_counts=0
    for char in word:
        for e in "abcdefghijklmnopqrstuvwxyz":
            if char==e:english_counts+=1
        for e in "абвгдеёжзийклмнопрстуфхцчшщъыьэюя":
            if char==e:russian_counts+=1
    if russian_counts==len(word):
        scores = russian_letters
    elif english_counts==len(word):
        scores = english_letters
    else:
...
print("Слово: ","word","- имеет количество очков: ",total_score,".")